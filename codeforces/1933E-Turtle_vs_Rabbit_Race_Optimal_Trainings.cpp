...
ll f(vll &pre, int mid, int l, ll u) {
  ll sect = (pre[mid] - pre[l - 1]);
  return -1 * sect * sect / 2 + sect / 2 + sect * u;
}
...
int main() {
  ...
  while(t--) {
    ...
    vll prefix(n + 1, 0);
    fn(i, n) prefix[i + 1] = prefix[i] + sects[i];
    ...
    while(q--) {
      ...
      while(l < r) {
        int mid = (l + r) / 2;
        if(f(prefix, mid, l_, u) < f(prefix, mid + 1, l_, u)) {
          l = mid + 1;
        } else {
          r = mid;
        }
