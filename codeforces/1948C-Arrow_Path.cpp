...
void dfs(vector< string > &grid, vvi &vis, int r, int p, int mode) {
  vis[r][p] += 1;
  // robot
  if(mode == 1) {
    if(r > 0 && vis[r][p] < 2) dfs(grid, vis, r - 1, p, 2);
    if(p < grid[0].size() - 1) dfs(grid, vis, r, p + 1, 2);
    if(r < 1 && vis[r][p] < 2) dfs(grid, vis, r + 1, p, 2);
  // auto
  } else {
    if(grid[r][p] == '<') return;
    else dfs(grid, vis, r, p + 1, 1);
  }
}
