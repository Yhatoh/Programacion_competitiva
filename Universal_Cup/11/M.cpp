...
int main() {
  ...
  while(t--) {
    ...
    SegmentTree< ll, merge2 > rightsum(nums);
    SegmentTree< ll, merge1 > maximums(nums);

    vll minimums(n);
    fn(i, n) {
      minimums[i] = min(maximums.query(0, i), maximums.query(i, n - 1));
    }

    DBG(minimums);
    SegmentTree< ll, merge2 > fg(minimums);
    int q;
    cin >> q;
    while(q--) {
      int a;
      ...
      a--;
      nums[a] += b;
      rightsum.update(a, nums[a]);
      maximums.update(a, nums[a]);
      fg.update(a, min(maximums.query(0, a), maximums.query(a, n - 1)));
      cout << fg.query(0, n - 1) - rightsum.query(0, n - 1) << "\n";
    }
  }
  return 0;
}
