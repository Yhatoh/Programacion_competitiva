...
struct Dinic {
  ...
  void add_edge(ll u, ll v, ll cap) {
    ...
    G[v].push_back({u, (ll)G[u].size() - 1, 0, cap}); // Use cap instead of 0 if bidirectional
  }
...
int main() {
  ...
  while(t--) {
    ...
    for (int a = 0; a < d.G.size(); a++) {
      for (auto b : d.G[a]) {
        if (b.f == b.c and (visited[a] ^ visited[b.to])) {
          cout << a + 1 << " " << b.to + 1 << endl;
        }
      }
    }
  }
