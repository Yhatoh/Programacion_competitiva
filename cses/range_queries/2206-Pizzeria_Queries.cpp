...
int main() {
  ...
  while(t--) {
    ...
    SegmentTree< ll, merge > STLeft(nums_1);
    SegmentTree< ll, merge > STRight(nums_2);
    fn(i, q) {
      int op;
      cin >> op;
      if(op == 1) {
        ...
        ll x;
        cin >> x;
        STLeft.update(k - 1, x - k);
        STRight.update(k - 1, x + k);
      } else {
        int k; cin >> k; k--;
        ll left = STLeft.query(0, k);
        ll right = STRight.query(k, n - 1);
